...
double nnn_energy(array_2d sitespin,  unsigned int row, unsigned int col);
	...
	for (unsigned int i = 1; i <=1e5 + N_mc; ++i)
	{
		for (unsigned int j = 1; j <= 3*sys_size/2; ++j)
		{
			...
			if (label <= sys_size)
			{
				...
				energy_diff=-2.0*nn_energy(sitespin1, J_x, J_y, row, col) -2.0*nnn_energy(sitespin1, row, col);

				if (row < axis1/2)
					energy_diff +=-2.0*nn_energy(sitespin2, J_x, J_y, row, col);
					energy_diff +=-2.0*nnn_energy(sitespin2, row, col);
			...
			if (label > sys_size)
			{
				...
				energy_diff=-2.0*nn_energy(sitespin2, J_x, J_y, row, col) -2.0*nnn_energy(sitespin2, row, col);

				if (row < axis1/2)
					energy_diff +=-2.0*nn_energy(sitespin1, J_x, J_y, row, col);
					energy_diff +=-2.0*nnn_energy(sitespin1, row, col);
...
double energy_tot(array_2d sitespin, array_2d J_x, array_2d J_y)
{
	...
	for (unsigned int i = 0; i < axis1 - 2; i=i+2)
	{
		for (unsigned int j = 0; j < axis2 - 2; j = j+2)
		{
			energy += Jp*sitespin[i][j]*sitespin[i+1][j];
			energy += Jp*sitespin[i][j]*sitespin[i][j+1];
		}
	}
	...
	for (unsigned int i = 0; i < axis1-1; ++i) // for j=axis2-1
	 {energy += J_y[i][axis2-1]*sitespin[i][axis2-1] * sitespin[i][0];
	  energy += J_x[i][axis2-1]*sitespin[i][axis2-1]*sitespin[i+1][axis2-1];
        ...
        for (unsigned int j = 0; j < axis2-2; j=j+2) // for i=axis1-1 and i=axis1-2
	 {energy += Jp*sitespin[axis1-2][j] * sitespin[0][j];
	  energy += Jp*sitespin[axis1-1][j] * sitespin[1][j];
	  energy += Jp*sitespin[axis1-2][j]*sitespin[axis1-2][j+2];
	  energy += Jp*sitespin[axis1-1][j]*sitespin[axis1-1][j+2];
	 }
	...
	for (unsigned int i = 0; i < axis1-2; ++i) // for j=axis2-1 and j=axis2-2
	 {energy += Jp*sitespin[i][axis2-2] * sitespin[i][0];
	  energy += Jp*sitespin[i][axis2-1] * sitespin[i][1];
	  energy += Jp*sitespin[i][axis2-2]*sitespin[i+2][axis2-2];
	  energy += Jp*sitespin[i][axis2-1]*sitespin[i+2][axis2-1];
	 }
	...
	energy += J_y[axis1-1][axis2-1]*sitespin[axis1-1][axis2-1]*sitespin[axis1-1][0];
        ...
        energy += Jp*sitespin[axis1-2][axis2-1]*sitespin[0][axis2-1];
        energy += Jp*sitespin[axis1-1][axis2-1]*sitespin[1][axis2-1];
        energy += Jp*sitespin[axis1-2][axis2-2]*sitespin[0][axis2-2];
        energy += Jp*sitespin[axis1-1][axis2-2]*sitespin[1][axis2-2];
	energy += Jp*sitespin[axis1-2][axis2-2]*sitespin[axis1-2][0];
	energy += Jp*sitespin[axis1-1][axis2-2]*sitespin[axis1-1][0];
	energy += Jp*sitespin[axis1-2][axis2-1]*sitespin[axis1-2][1];
	energy += Jp*sitespin[axis1-1][axis2-1]*sitespin[axis1-1][1];
...
double nnn_energy(array_2d sitespin , unsigned int row, unsigned int col)
{
	...
	nnn_en +=Jp * sitespin[row % axis1][col % axis2] * sitespin[(row + 2) % axis1][col % axis2];
        nnn_en +=Jp * sitespin[row % axis1][col % axis2] * sitespin[(row - 2) % axis1][col % axis2];
	nnn_en +=Jp * sitespin[row % axis1][col % axis2] * sitespin[row % axis1][(col - 2) % axis2];
	nnn_en +=Jp * sitespin[row % axis1][col % axis2] * sitespin[row % axis1][(col + 2) % axis2];
	return nnn_en;
}
